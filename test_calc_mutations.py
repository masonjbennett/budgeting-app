...
MUTATIONS = [
    ...
    ('a stable value contract is reported as a collective trust, which sends somebody looking for a fee disclosure that describes a different vehicle',
     ...
     "fund_kinds.py"),
    # A registered trust is not a collective one, and by NAME the two can be
    # the same shape. Found by an outside review, reproduced on nine names.
    ('a word beginning with a class letter after "Trust" counts as the class marker, so "Trust Value" and "Trust Income" are called collective trusts',
     (r'    r"\btrust,?\s+(i{1,3}|iv|v|select|unit|class|[a-z])\b|"',
      r'    r"\btrust,?\s+(i{1,3}|iv|v|select|unit|class|[a-z]\b)|"'),
     "fund_kinds.py"),
    ('a name ending in "Trust" is called a collective trust even beside a ticker, which tells a holder of a gold trust it has none',
     ('    if len(lone) == 1 and _TRUST_ONLY.search(lone[0]):',
      '    if any(_TRUST_ONLY.search(s) for s in lone):'),
     "fund_kinds.py"),
    ('the hedged note is dropped, so a name the tool cannot tell from a registered trust is described with the certainty of one it can',
     ('        return COLLECTIVE_TRUST, NOTES[TRUST_BY_NAME]',
      '        return COLLECTIVE_TRUST, NOTES[COLLECTIVE_TRUST]'),
     "fund_kinds.py"),
    ('a fund the table carries without a ratio is reported like one the table lacks, so the page sends somebody checking a ticker that is right',
     ('                     for r in rows if r["er"] is None and r["known"]],',
      '                     for r in rows if False],')),
